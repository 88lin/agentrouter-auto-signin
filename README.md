...
- 直连访问站点，**不需要配代理**（开了 TUN 也一样，脚本无感知）
- 幂等：重复运行只会显示「今日已签到」，不会多领
...
看到 `"result": "OK"` 或 `"ALREADY"`、`report` 里报出余额，就通了。之后挂上定时任务即可。
...
> 为什么是两个？站点没有公开每日签到的重置时点，而且早上电脑可能没开机。脚本幂等，多跑一次成本几乎为零，却能兜住「重置时点不确定」和「错过开机」两种情况。
...
**网络不用配置**：脚本直连站点，没有代理项。开着 Clash 的 TUN 模式也一样——那是系统层接管流量，脚本无感知。
...
| `NO_EXIT` | 站点返回了 WAF 拦截页。**先确认自己没开着代理**——代理出口是机房 IP 时最容易命中站点风控；关掉走直连通常就好了 |
...
- **不要专门给它套代理。** 流量交给机场节点，出口就变成机房 IP，而站点用阿里云 WAF 拦机房 IP——本来直连能通，挂了代理反而不通。
- 重复运行是安全的，签到由服务端按天判定，不会多领。
